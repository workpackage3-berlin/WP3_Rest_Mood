...
try:
    ...
    if (len(discoveryList) > 0):
        ...
        dev.import_configuration(join(configs_dir, "NEW1_eeg_config_changed_ref.xml"))
        ...
        plotter_helper = ImpedancePlotterHelper(device=dev,
                                                 layout='head', 
                                                 file_storage = join(measurements_dir,"rest_mood_impedances"))
        ...
        print('\n Saving impedances in the measurements folder under "rest_mood_impedances-date-time"... \n')
        time.sleep(1)
